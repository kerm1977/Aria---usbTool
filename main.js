...
ipcMain.handle('list-usb-devices', async () => {
  try {
    const result = await runShell('lsblk -J -o NAME,SIZE,TYPE,MOUNTPOINT,MODEL,LABEL,FSTYPE,RM,STATE', 10000);
    if (result.code !== 0) throw new Error(result.stderr || 'Error listando dispositivos');
    const data = JSON.parse(result.stdout);
    const usbDevices = data.blockdevices.filter((d) => {
      const isRemovable = d.rm === true || d.rm === 1 || d.rm === '1';
      const hasUsbModel = d.model && d.model.toLowerCase().includes('usb');
      const realSize = parseSize(d.size) > 1024 * 1024; // mas de 1MB
